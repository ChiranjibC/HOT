...
{
    public class _BatchController : BaseController
    {
        private IBatch _batch;
        private IProduct _product;
        public _BatchController(IBatch batch, IProduct product)
        {
            _batch = batch;
            _product = product;
        }
        ...
        public ActionResult Edit(string id)
        {
            ...
            var batch = guid != Guid.Empty ? _batch.GetDetails(guid.ToString()) : new BatchViewModel();
            //batch.Products = GetProductSelectList();            
            //batch.OwnerList = GetOwnersSelectList();
            return PartialView(batch);
        }

        private SelectList GetOwnersSelectList()
        {
            return new SelectList(_batch.GetOwnersList(),"OwnerId","OwnerDesc");
        }
        ...
        public ActionResult Edit(BatchViewModel batchViewModel)
        {
            try
            {
                ...
                return Json(new { Success = true, updatedBatch.BatchCode });
            }
            catch
            {
                return View();
